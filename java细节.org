...
3. 常见方法
   ...
   2. 写入文件
	 ...
	 =如果含有中文，保存的是unicode码值=
      2. properties.store(new FileOutputStream("src\\mysql2.properties"), null);
	 null是显示在properties文件中的注释
* JDBC
JDBC：接口(java厂商制定规范) mysql实现类 -> jar(mysql驱动)
...
2. 获取连接-得到Connection
   String url = "jdbc:mysql://LocaThost:3306/数据库名
   jdbc:mysql: 协议
   Connection connect = driver.connect(urL, properties对象)
   =properties对象: 规定key: user password=
3. 执行增删改查-发送SQL给mysqI执行
...
4. 释放资源-关闭相关连接
   ...
   connect.close()
*** 获取数据库连接5种方式
1. Connection connect = driver.connect(urL, properties对象)
2. Class<?> aClass = Class.forName("com.mysqL.jdbc.Driver")
   Driver driver = (Driver)aclass.newInstance();
3. 使用 DriverManager 替代 Driver，直接将 url user password 当做参数，不用创建properties对象
   DriverManager.registerDriver(driver)
   Connection connection = DriverManager.getConnection(urL,user, password);
4. 可以省略 registerDriver
   在加载Driver类时，在Driver的静态代码块完成注册
5. 从配置文件读取
** ResultSet 结果集
1. 表示数据库结果集的数据表，通常通过执行 =查询= 数据库的语句生成
...
2. int id = resultSet.getInt(1)
   获取该行的第1列数据
3. string string = resultSet.getString(2)
   获取该行的第2列数据
4. 既也可以根据第几列取出数据，也可以根据列名取出数据
*** 底层是个rows是个ArrayList(可变，因为不知道有多少行)，ArrayList里面包含elementData，是个Object数组(不可变，每行所占空间相同)，这个对象数组里面包含了各行的数据
** PreparedStatement
好处：防止sql注入，并且编译了sql语句，提高了效率
1. PreparedStatement preparedstatement = connection.prepareStatement(带占位符sql语句);
2. 给占位符赋值，占位符是 =从1开始计算的=
   preparedStatement.setstring(1, admin_name)
3. 执行select语句使用executeQuery. 如果执行的是dml(update, nsert,delete) executeUpdate()
** 批处理
1. 当需要成批插入或者更新记录时。可以采用Java的批量更新机制，这一机制允许多条语句一次性提交给数据库批量处理。通常情况下比单独提交处理更有效率。
2. JDBC的批量处理语句包括下面方法:
   - addBatcn):添加需要批量处理的SQL语句或参数
   - executeBatch():执行批量处理语句;
   - clearBatch():清空批处理包的语句a, =用来清理已执行完毕的sql=
...
3. 当应用程序向连接池请求的连接数超过最大 连接数量时，这些请求将被加入到等待队列中
** C3P0
1. 创建一个数据源对象
   ComboPooLedDataSource comboPooLedDataSource = new ComboPooLedDataSource();
   ComboPooLedDataSource 实现了 DataSouce
2. 给数据源设置数据库相关参数
...
3. 设置初始化连接数
   1. 初始化连接数
      comboPooledDataSource.setInitialPoblSize(10);
   2. 最大连接数(如果达到初始化连接数，之后还能创建的连接数,此时还不会进入等待队列)
      comboPooLedDataSource.setMaxPooLSize(50);
4. comboPooLedDataSource.getConnection();
5. connection.close();
*** 配置文件方式
1. 将c3p0提供的c3p0-config.xml(名称是固定的，不能更改) 拷贝到src目录下
2. ComboPooLedDataSource comboPooledDataSource = new ComboPooLedDataSource(配置文件当中配置的name)
** Druid(德鲁伊)
1. 加入配置文件 druid.properties(名称可以更改) 将该文件拷贝项目的src目录
2. 创建 Properties 对象，读取配置文件
3. DruidDataSourceFactory.createDataSource(properties对象)
4. getConnection()
5. close()
* Apache--DBUtils
为什么需要：关闭connection 后，resultSet 结果集无法使用
- 土方法完成封装
  1. 将结果集记录,封装到ArrayList<Actor>中
  2. 一个actor对象对应一条表记录，actor对象放入到ArrayList集合
** 查询
1. 先引入DButils相关的jar
2. List<Actor> list = new QueryRunner.query(connect, sql, new BeanListHandler<>(Actor.class), 1)
   - new BeanListHandler<>(Actor.class)
     将resultset 转为 bean对象 封装到ArrayList
   - 1
     给sql 占位符赋值，可以有多个值，是个可变参数
3. 返回单行记录(单个对象): new BeanHandler<>(Actor.cLass)
   返回的是一个bean对象
4. 返回单行单列记录: new ScalarHandLer()
   返回的是object对象
*** 结果集
** DML
- 更改
...
* 数据库连接池和DBUtils的区别
1. 数据库连接池是获取连接的，可以复用连接
2. DBUtils是对JDBC(Java Database Connectivity)的封装，用在获取连接之后，封装了 statement(执行sql语句) 和 ResultSet(第次调用next()，返回一个set集合)
3. DBUtils返回的结果是对resultSet是封装，类型根据第3个参数决定的，否则返回的结果是 resultSet，是一个可以遍历的集合，
4. 使用DBUtils就可以不用关闭 statement，因为DBUtils会帮我们关闭
* BasicDao
1. SQL语句是固定，不能通过参数传入，通用性不好，需要进行改进，更方便执行增删改查
2. 对于select 操作，如果有返回值,返回类型不能固定，需要使用泛型
3. 将来的表很多，业务需求复杂，不可能只靠一个Java类完成
* 回顾BasicDao
BasicDao是对Dao层共同代码的封装
